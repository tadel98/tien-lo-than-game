...
process.env.DATABASE_URL = "postgresql://neondb_owner:npg_u5SXZm3yzvOl@ep-long-dust-a1cl7wyx-pooler.ap-southeast-1.aws.neon.tech/neondb?sslmode=require&channel_binding=require"
...
async function testDatabase() {
  console.log('🧪 Bắt đầu test database với Neon PostgreSQL...')
  ...
  try {
    ...
    const expGain = 1000
    const newExp = Number(testPlayer.experience) + expGain
    ...
    if (resources.length > 0) {
      ...
      resources.forEach(resource => {
        console.log(`  - ${resource.displayName} (${resource.name}): ${resource.icon}`)
      })
    }
