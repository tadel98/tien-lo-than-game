...
export function calculateCombatPowerWithLevel(player) {
  ...
  const totalStats = {
    ...
    intelligence: (baseStats.intelligence || 0) + Math.floor((baseStats.intelligence || 0) * 0.1 * level),
    vitality: (baseStats.vitality || 0) + Math.floor((baseStats.vitality || 0) * 0.1 * level),
    hp: (baseStats.hp || 0) + Math.floor((baseStats.hp || 0) * 0.05 * level),
    mp: (baseStats.mp || 0) + Math.floor((baseStats.mp || 0) * 0.05 * level),
    ...
    player.equipments.forEach((equipment) => {
      if (equipment.equipment && equipment.equipment.stats) {
        ...
        equipmentBonus += (eqStats.strength || 0) + (eqStats.agility || 0) + 
                         (eqStats.intelligence || 0) + (eqStats.vitality || 0) +
                         (eqStats.attack || 0) + (eqStats.defense || 0)
      }
    })
  }
  
  // Công thức tính sức mạnh chiến đấu
  const basePower = totalStats.strength + totalStats.agility + totalStats.intelligence + totalStats.vitality +
                   totalStats.hp + totalStats.mp + totalStats.attack + totalStats.defense + totalStats.speed
